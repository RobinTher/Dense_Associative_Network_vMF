...
class AltOneNorm(Constraint):
    ...
    def alt_normalize(self, g, not_all_normalized):
        ...
        not_all_normalized = k.any(k.abs(g_norm - 1) > k.epsilon()**(1/2))

        g = self.counts_memory[: self.input_size] * norm.tensor_normalize(g, g_norm, sub_value = self.prior_y)
        g_0 = self.counts_memory[self.input_size] * self.prior_y

        ratio = (self.input_size + 1) / (self.max_input_size + 1)
        # ratio = 1
        g_norm = norm.tensor_one_norm(g, axis = 0) + g_0
        g = (self.input_size + ratio) * self.prior_y * norm.tensor_normalize(g, g_norm)
    ...
    def __call__(self, g):
        ...
        g[: self.input_size].assign(tf.while_loop(self.keep_looping, self.alt_normalize, [g[: self.input_size], not_all_normalized],
                                                  maximum_iterations = self.number_iterations)[0])
        g[self.input_size].assign(self.counts_memory[self.input_size] * self.prior_y)
        
        # for _ in range(self.number_iterations):
            # g_norm = norm.tensor_one_norm(g[: self.input_size], axis = 1)
            
            # not_all_normalized = k.any(k.abs(g_norm - 1) > k.epsilon())
            # iteration += 1
            # tf.debugging.assert_all_finite(g, message = "Normalization failed.")
            
            # g[: self.input_size].assign(self.counts_memory[: self.input_size] * norm.tensor_normalize(g[: self.input_size], g_norm, sub_value = self.prior_y))
            # g[self.input_size].assign(self.counts_memory[self.input_size] * self.prior_y)
            
            # max_input_size = g.shape[0]
            # ratio = 1 # (self.input_size + 1) / (max_input_size + 1)
            # g_norm = norm.tensor_one_norm(g, axis = 0)
            # g.assign((self.input_size + ratio) * self.prior_y * norm.tensor_normalize(g, g_norm))
