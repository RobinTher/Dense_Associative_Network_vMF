...
class LogDenseExp(Layer):
    ...
    def build(self, input_shape):
        ...
        self.counts_memory[self.input_size].assign(ratio)
    ...
    def call(self, h):
        ...
        def silent_normalization(kernel):
            def grad(upstream):
                ...
                downstream = ((self.input_size + ratio) * self.prior_y - kernel) * (self.counts_memory[: self.input_size + 1] - kernel) / ((self.input_size + ratio) * self.prior_y * self.counts_memory[: self.input_size + 1] - kernel**2) * upstream
                
                # upstream_mean = k.mean(upstream * kernel, axis = 0, keepdims = True)
                
                # downstream = upstream + self.prior_y - upstream_mean / kernel + (self.counts_memory[: self.input_size + 1] - k.sum(upstream * kernel, axis = 1, keepdims = True)) / (self.output_size + 1) / kernel - (1 - k.sum(upstream_mean)) / (self.output_size + 1) / kernel
